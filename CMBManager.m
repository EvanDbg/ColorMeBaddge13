...
- (CMBColorInfo *)getPreferredAppBadgeColorsForIcon:(CMBIconInfo *)iconInfo
{
	...
	if (iconInfo.isApplication)
	{
		...
		iconImage = [iconInfo.icon iconImageWithInfo:imageInfo];

		if ([[CMBPreferences sharedInstance] useUnmaskedIcons])
		{
			UIImage *unmaskedIconImage = [iconInfo.icon unmaskedIconImageWithInfo:imageInfo];
...
{
	if (![[objc_getClass("SBIconController") sharedInstance] allowsBadgingForIcon:iconInfo.icon])
		return nil;
